...
const SettingsPage = () => {
  ...
  const exportData = async () => {
    ...
    try {
      ...
      setPreparedExport(downloadJson({
    ...
    <Page title="Settings" backHref="/dashboard">
      ...
      <IonCard>
        <IonCardContent>
          ...
          <IonButton expand="block" fill="outline" disabled={exporting} onClick={() => void exportData()}>{exporting ? 'Preparing export…' : 'Export my data'}</IonButton>
          {preparedExport && <p className="legal-note">If the download did not open automatically, <a href={preparedExport.url} download={preparedExport.name}>download the prepared export</a>.</p>}
      ...
      <IonCard><IonCardContent><CaregiverCollaboration /></IonCardContent></IonCard>
