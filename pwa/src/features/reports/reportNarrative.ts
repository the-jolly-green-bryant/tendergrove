...
export const buildNarrativeEnvelope = (report: ProviderReport): NarrativeEnvelope => {
    ...
    ? [
        {
          ...
          replacement: difficultToSustain
            ? 'The recent pattern shows repeated regression and sustained concerning observations, which does not look sustainable.'
            : recentTrendImproved
  ...
  if (report.baseline !== null && recentRegressiveDays.length) {
    facts.push({
      ...
      meaning:
        'HIGH PRIORITY: explain what the recent regressive-day concentration suggests.',
      replacement: `${recentRegressiveDays.length} of ${recent.length} recent recorded observations fell below the ${report.baseline}-point baseline (${recentRegressiveRate}%, ${baselineDelta(regressiveRateDelta)})${recentRegressiveStretch >= 2 ? `; longest stretch: ${recentRegressiveStretch} days` : ''}. This suggests a repeated regression rather than an isolated decrease.`,
    })
  }
  ...
  if (recent.length) {
    ...
    facts.push({
      ...
      replacement: `${recent.length} recent recorded observations were available. ${report.recent === null || report.baseline === null ? 'A baseline comparison is not yet available.' : `Recent wellness was ${report.recent} points versus the ${report.baseline}-point baseline.`}${recentDifficult[0] ? ` Longest stretch of concerning observations: ${recentDifficult[0].days} days.` : ''}${recentPositive[0] ? ` Longest stretch of steady observations: ${recentPositive[0].days} days.` : ''}`,
    })
  ...
  if (report.eventComparisons.length) {
    ...
    facts.push({
      id: 'event_summary',
      meaning:
        'Concise provider-facing summary of the strongest recorded event association without implying causation.',
  ...
  if (strainIsNoteworthy) {
    ...
    facts.push({
      ...
      replacement: `${PATTERN_STRAIN_LABELS[strain.band]} is driven most by ${strongest.map(([label, value]) => `${patternDimensionLevel(value).toLowerCase()} ${label}`).join(' and ')}.${strain.largestDecline >= 15 ? ` The largest recent decline was ${strain.largestDecline} points.` : ''}`,
    })
  }
...
export const validateNarrativeTemplate = (
  ...
  const recentRatePattern =
    /\(\d+%, (?:unchanged from baseline|\d+ points (?:up|down) from baseline)\)/
