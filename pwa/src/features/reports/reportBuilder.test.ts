...
describe('provider report', () => {
  it('states limitations and does not turn missing days into wellness data', () => {
    ...
    expect(report.text).toContain('1% of the selected 90-day window has recorded data')
    expect(report.text).toContain('3% of the selected 30-day window has recorded data')
  ...
  it('compares any sufficiently recorded event without hardcoding passes', () => {
    ...
    expect(report.text).toContain('and a 90-day baseline of')
