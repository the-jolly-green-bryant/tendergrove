...
/*
AI-generated initial attempt at report generation.
 */
export function buildPlainTextReport(
  ...
  const lines = [
    ...
    ...checkIns
      ...
      .map(
        (x) =>
          `- ${new Date(x.createdAt).toLocaleDateString()}: mood=${x.mood}, distress=${x.severity}, schoolDay=${x.schoolDay}, toiletingChange=${x.toiletingChange}${x.notes ? `, notes=${x.notes}` : ''}`,
      ),
    '',
    'Recent incidents',
    ...incidents
      ...
      .map(
        (x) =>
          `- ${new Date(x.createdAt).toLocaleDateString()}: severity=${x.severity}, duration=${x.durationMinutes}m, behavior=${x.behavior}, trigger=${x.trigger ?? 'unknown'}, intervention=${x.intervention ?? 'none noted'}, recovered=${x.recovered}`,
      ),
  ]
  return lines.join('\n')
}
