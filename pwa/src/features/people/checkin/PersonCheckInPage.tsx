...
export default function PersonCheckInPage() {
  ...
  const indicators = useMemo(
    () =>
      ((person?.indicators ?? []) as Indicator[]).filter(
    ...
    <IonPage>
      <IonHeader translucent>
        <IonToolbar>
          ...
          <IonTitle>
            {person
              ? `${person.displayName} — ${formatDateLabel(selectedDate)}`
              : 'Check-In'}
          </IonTitle>
        </IonToolbar>
      </IonHeader>
      ...
      >
        ...
        {person && (
          <>
            <p className="check-in__intro">
              Check off everything you saw{' '}
              {formatDateLabel(selectedDate).toLowerCase() === 'today'
                ? 'today'
