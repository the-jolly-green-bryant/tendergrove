...
async function seed() {
  ...
  const undesiredIndicators = indicatorIds.filter(
    (i) => i.meta.polarity === 'undesired',
  )
  ...
  for (let daysAgo = 364; daysAgo >= 0; daysAgo--) {
    ...
    if (result.errors?.length) {
      console.error(
        `  ✗ Check-in ${date.toISOString().slice(0, 10)}: ${result.errors[0].message}`,
      )
    } else {
      created++
      if (created % 30 === 0) {
